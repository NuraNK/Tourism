...
class Hotels(models.Model):
    ...
    gallery = models.ManyToManyField(Gallery, related_name='gallery_hotel',
...
class RoomsHotel(models.Model):
    ...
    gallery = models.ManyToManyField(Gallery, related_name='gallery_room',
                                     null=True, blank=True)
...
class ReviewTotal(TimeStampedModel):
    ...
    total = models.IntegerField(default=0)
    hotel = models.ForeignKey(Hotels, on_delete=models.CASCADE,
                              related_name='room_totals')
    room = models.ForeignKey(RoomsHotel, on_delete=models.CASCADE,
                             related_name='room_totals')
...
class RateHotels(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        review_total = ReviewTotalHotel.objects.filter(
        ...
        if review_total:
            ...
            ReviewTotalHotel.objects.create(rate=self.rate, all=self.rate,
...
class RateRoom(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        review_total = ReviewTotal.objects.filter(hotel=self.hotel,
        ...
        if review_total:
            ...
            ReviewTotal.objects.create(rate=self.rate, all=self.rate, total=1,
